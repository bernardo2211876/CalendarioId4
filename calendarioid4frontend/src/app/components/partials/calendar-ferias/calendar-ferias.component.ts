...
@Component({
  selector: 'app-calendar-ferias',
  templateUrl: './calendar-ferias.component.html',
  styleUrls: ['./calendar-ferias.component.css']
})
export class CalendarFeriasComponent implements OnInit{
  ...
  carregarTeletrabalho(){
    if (this.data.id) {
      this._ausenciaService.getFerias(this.data.id).subscribe({
        next: (res:any[]) => {
          const newEvents = res.map(event=>{
            ...
            return {
              ...
              actions: [
                {
                  label: '<i class="fa-solid fa-eye" style="color: #ffffff;"></i>',
